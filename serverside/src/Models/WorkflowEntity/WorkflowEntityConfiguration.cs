...
namespace Utawalaaltar.Models {
	public class WorkflowEntityConfiguration : IEntityTypeConfiguration<WorkflowEntity>
	{
		public void Configure(EntityTypeBuilder<WorkflowEntity> builder)
		{
			...
			// % protected region % [Override Versionss Workflow configuration here] off begin
			builder
				.HasMany(e => e.Versionss)
				.WithOne(e => e.Workflow)
				.OnDelete(DeleteBehavior.Cascade);
			// % protected region % [Override Versionss Workflow configuration here] end

			// % protected region % [Override CurrentVersion CurrentWorkflow configuration here] off begin
			builder
				.HasOne(e => e.CurrentVersion)
				.WithOne(e => e.CurrentWorkflow)
				.OnDelete(DeleteBehavior.Restrict);
			// % protected region % [Override CurrentVersion CurrentWorkflow configuration here] end
