...
namespace Utawalaaltar.Models {
	...
	public class ProtocolEntity : User, IOwnerAbstractModel 
	// % protected region % [Modify class declaration here] end
	{
		...
		[EntityForeignKey("Member", "Protocol", true, typeof(MembersEntity))]
		...
		public async override Task<int> CleanReference<T>(
			...
			{
			var modelList = models.Cast<ProtocolEntity>().ToList();
