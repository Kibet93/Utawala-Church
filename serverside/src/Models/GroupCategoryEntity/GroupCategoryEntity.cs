...
namespace Utawalaaltar.Models {
	...
	public class GroupCategoryEntity : User, IOwnerAbstractModel 
	// % protected region % [Modify class declaration here] end
	{
		...
		// % protected region % [Customise CategoryGroupLeaderss here] off begin
		/// <summary>
		/// Incoming one to many reference
		/// </summary>
		/// <see cref="Utawalaaltar.Models.CategoryGroupLeaderEntity"/>
		[EntityForeignKey("CategoryGroupLeaderss", "GroupCategory", false, typeof(CategoryGroupLeaderEntity))]
		public ICollection<CategoryGroupLeaderEntity> CategoryGroupLeaderss { get; set; }
		// % protected region % [Customise CategoryGroupLeaderss here] end

		// % protected region % [Customise Memberscategoriess here] off begin
		/// <summary>
		/// Incoming one to many reference
		/// </summary>
		/// <see cref="Utawalaaltar.Models.MemberEntity"/>
		[EntityForeignKey("Memberscategoriess", "GroupCategory", false, typeof(MemberEntity))]
		public ICollection<MemberEntity> Memberscategoriess { get; set; }
		// % protected region % [Customise Memberscategoriess here] end
		...
		public async override Task<int> CleanReference<T>(
			...
			{
			var modelList = models.Cast<GroupCategoryEntity>().ToList();
			var ids = modelList.Select(t => t.Id).ToList();

			switch (reference)
			{
				case "Memberscategoriess":
					var memberscategoriesIds = modelList.SelectMany(x => x.Memberscategoriess.Select(m => m.Id)).ToList();
					var oldmemberscategories = await dbContext.MemberEntity
						.Where(m => m.GroupCategoryId.HasValue && ids.Contains(m.GroupCategoryId.Value))
						.Where(m => !memberscategoriesIds.Contains(m.Id))
						.ToListAsync(cancellation);

					foreach (var memberscategories in oldmemberscategories)
					{
						memberscategories.GroupCategoryId = null;
					}

					dbContext.MemberEntity.UpdateRange(oldmemberscategories);
					return oldmemberscategories.Count;
