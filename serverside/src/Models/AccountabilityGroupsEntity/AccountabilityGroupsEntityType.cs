...
namespace Utawalaaltar.Models
{
	...
	/// </summary>
	public class AccountabilityGroupsEntityType : ObjectGraphType<AccountabilityGroupsEntity>
	{
		public AccountabilityGroupsEntityType()
		{
			...
			Field(o => o.Category, type: typeof(IntGraphType));
			...
			// GraphQL reference to entity MembersEntity via reference Membersaccountabilitygroup
			Field<ListGraphType<NonNullGraphType<MembersEntityType>>, IEnumerable<MembersEntity>>()
				...
				.ResolveAsync(async context =>
				{
					...
					var loader = accessor.Context.GetOrAddCollectionBatchLoader<Guid?, MembersEntity>(
						string.Join("-", context.ResponsePath.Where(x => x is string)) + "GetMembersaccountabilitygroupsForAccountabilityGroupsEntity",
						async keys =>
						{
							...
							var results = await query
								.Where(x => x.AccountabilityGroupId.HasValue && keys.Contains(x.AccountabilityGroupId))
								.Select(x => x.AccountabilityGroupId.Value)
								.Distinct()
								.SelectMany(x => query
									.Where(y => y.AccountabilityGroupId == x)
							...
							return results.ToLookup(x => x.AccountabilityGroupId, x => x);
						});
	...
	/// </summary>
	public class AccountabilityGroupsEntityInputType : InputObjectGraphType<AccountabilityGroupsEntity>
	{
		public AccountabilityGroupsEntityInputType()
		{
			Name = "AccountabilityGroupsEntityInput";
			Description = "The input object for adding a new AccountabilityGroupsEntity";
			...
			Field<ListGraphType<MembersEntityInputType>>("Membersaccountabilitygroups");
