...
namespace Utawalaaltar.Models {
	...
	// % protected region % [Modify class declaration here] end
	{
		...
		[EntityForeignKey("Membersaccountabilitygroups", "AccountabilityGroup", false, typeof(MembersEntity))]
		...
		public async Task<int> CleanReference<T>(
			...
			{
			var modelList = models.Cast<AccountabilityGroupsEntity>().ToList();
			...
			switch (reference)
			{
				case "Membersaccountabilitygroups":
					...
					var oldmembersaccountabilitygroup = await dbContext.MembersEntity
					...
					dbContext.MembersEntity.UpdateRange(oldmembersaccountabilitygroup);
