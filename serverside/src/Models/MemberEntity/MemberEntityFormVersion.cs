...
namespace Utawalaaltar.Models {
	...
	// % protected region % [Modify class declaration here] end
	{
		...
		public MemberEntityFormVersion()
		{
		...
		[EntityForeignKey("Form", "FormVersions", false, typeof(MemberEntity))]
		public MemberEntity Form { get; set; }

		/// <summary>
		/// The current form that this version is published against if it is published
		/// </summary>
		/// <see cref="Utawalaaltar.Models.MemberEntity"/>
		[EntityForeignKey("PublishedForm", "PublishedVersion", false, typeof(MemberEntity))]
		public MemberEntity? PublishedForm { get; set; }

		/// <summary>
		/// Reference to the submissions for this form version
		/// </summary>
		/// <see cref="Utawalaaltar.Models.MemberSubmissionEntity"/>
		[EntityForeignKey("FormSubmissions", "FormVersion", false, typeof(MemberSubmissionEntity))]
		public ICollection<MemberSubmissionEntity> FormSubmissions { get; set; }
			...
			CancellationToken cancellationToken = default)
		{
			...
			if (PublishVersion)
			{
				var formModel = dbContext.MemberEntity.FirstOrDefault(m => m.Id == FormId);
				if (formModel != null)
				{
		...
		public async Task<int> CleanReference<T>(
			...
			{
			var modelList = models.Cast<MemberEntityFormVersion>().ToList();
