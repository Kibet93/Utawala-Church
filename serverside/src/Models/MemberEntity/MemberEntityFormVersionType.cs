...
namespace Utawalaaltar.Models
{
	...
	/// </summary>
	public class MemberEntityFormVersionType : ObjectGraphType<MemberEntityFormVersion>
	{
		public MemberEntityFormVersionType()
		{
			Description = @"The form versions for the MEMBER Entity form behaviour";
			...
			Field<MemberEntityType, MemberEntity>()
				...
				.ResolveAsync(async context =>
				{
					...
					var loader = accessor.Context.GetOrAddBatchLoader<Guid?, MemberEntity>(
						"GetPublishedFormForMemberEntityFromVersion",
						async keys =>
						{
			...
			Field<MemberEntityType, MemberEntity>()
				...
				.ResolveAsync(async context =>
				{
					...
					var accessor = graphQlContext.ServiceProvider.GetRequiredService<IDataLoaderContextAccessor>();
					var loader = accessor.Context.GetOrAddBatchLoader<Guid, MemberEntity>(
						"GetFormForMemberEntityFormVersion",
						async keys =>
						{
							var results = await QueryHelpers.BuildQueryResolver<MemberEntity>(
								context,
			...
			Field<ListGraphType<MemberSubmissionEntityType>, IEnumerable<MemberSubmissionEntity>>()
				...
				.ResolveAsync(async context =>
				{
					var graphQlContext = (UtawalaaltarGraphQlContext) context.UserContext;
					var accessor = graphQlContext.ServiceProvider.GetRequiredService<IDataLoaderContextAccessor>();
					var loader = accessor.Context.GetOrAddCollectionBatchLoader<Guid, MemberSubmissionEntity>(
						"GetFormSubmissionsForMemberEntityFormVersion",
						async keys =>
						{
							var args = new CommonArguments(context);
							var query = QueryHelpers.CreateResolveFunction<MemberSubmissionEntity>(context, new ReadOptions {DisableAudit = true});
	...
	/// </summary>
	public class MemberEntityFormVersionInputType : InputObjectGraphType<MemberEntityFormVersion>
	{
		public MemberEntityFormVersionInputType()
		{
			Name = "MemberEntityFormVersionInput";
			Description = "The input object for adding a new MemberEntityFormVersion";
			...
			Field<ListGraphType<MemberSubmissionEntityInputType>>("FormSubmissions").Description = @"The submissions for this form version";
