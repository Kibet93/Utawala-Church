...
namespace Utawalaaltar.Models {
	...
	public class CategoryLeadersEntity : User, IOwnerAbstractModel 
	// % protected region % [Modify class declaration here] end
	{
		...
		[EntityForeignKey("Member", "CategoryGroupLeader", true, typeof(MembersEntity))]
		...
		public async override Task<int> CleanReference<T>(
			...
			{
			var modelList = models.Cast<CategoryLeadersEntity>().ToList();
