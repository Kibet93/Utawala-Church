...
namespace Utawalaaltar.Models {
	...
	public class MembersEntity : User, IOwnerAbstractModel 
	// % protected region % [Modify class declaration here] end
	{
		...
		[EntityForeignKey("Picture", "MembersPicture", false, typeof(UploadFile))]
		...
		/// </summary>
		/// <see cref="Utawalaaltar.Models.AccountabilityGroupsEntity"/>
		public Guid? AccountabilityGroupId { get; set; }
		[EntityForeignKey("AccountabilityGroup", "Membersaccountabilitygroups", false, typeof(AccountabilityGroupsEntity))]
		...
		/// </summary>
		/// <see cref="Utawalaaltar.Models.CategoryLeadersEntity"/>
		[EntityForeignKey("CategoryGroupLeader", "Member", false, typeof(CategoryLeadersEntity))]
			...
			CancellationToken cancellationToken = default)
		{
			...
			if (operation == EntityState.Deleted)
			{
				...
				{
					var existingFile = dbContext.Files.FirstOrDefault(f => f.Id == PictureId.Value);
		...
		public async override Task<int> CleanReference<T>(
			...
			{
			var modelList = models.Cast<MembersEntity>().ToList();
