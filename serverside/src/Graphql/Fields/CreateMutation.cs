...
namespace Utawalaaltar.Graphql.Fields
{
	public class CreateMutation
	{
		...
		public static Func<IResolveFieldContext<object>, Task<object>> CreateCreateMutation<TModel>(string name)
			where TModel : class, IOwnerAbstractModel, new()
		{
			...
			return async context =>
			{
				...
				try
				{
					...
					var data = await crudService.Create(models, new UpdateOptions
					{
		...
		/// <returns>A function that takes a graphql context and returns a list of created models</returns>
		public static Func<IResolveFieldContext<object>, Task<object>> CreateUserCreateMutation<TModel, TRegisterModel, TGraphQlRegisterModel>(string name)
