...
import AttributeWorkflowData from "./AttributeWorkflowData";
	...
	onChangeAndBlur?: (attributeName: string) => void)
{
	...
	switch (displayType) {
		...
		case 'workflow-data':
			// % protected region % [Override workflow-data here] off begin
			return <AttributeWorkflowData
				key={attributeOptions.attributeName}
				model={model}
				className={className}
				options={attributeOptions}
				...
				formMode={formMode}
			/>;
			// % protected region % [Override workflow-data here] end
		case 'hidden':
