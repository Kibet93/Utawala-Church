...
{
	public class CategoryLeadersEntity : UserBaseEntity 
	{
		// 
		[EntityAttribute]
		...
		public CategoryLeadersEntity()
		{
			EntityName = "CategoryLeadersEntity";
			EndpointName = "category-leaders-entity";
			InitialiseReferences();
		}

		public CategoryLeadersEntity(ConfigureOptions option)
		{
			EndpointName = "category-leaders-entity";
			Configure(option);
			InitialiseReferences();
		...
		{
			return GetInvalidEntities<CategoryLeadersEntity>()
		...
		public override Dictionary<string, string> ToDictionary()
		{
			var entityVar = new Dictionary<string, string>()
			{
				...
				{"email" , EmailAddress},
				{"password" , Password},
				{"categoryGroup" , CategoryGroup.ToString()},
			};
		...
		public override RestSharp.JsonObject ToJson()
		{
			...
			{
				entityVar["categoryGroup"] = CategoryGroup.ToString();
			}
			if (MemberId  != default)
			{
				entityVar["memberId"] = MemberId.ToString();
		...
		public static CategoryLeadersEntity GetEntity(bool isValid, string fixedValue = null)
		{
			if (isValid && !string.IsNullOrEmpty(fixedValue))
			{
				return GetValidEntity(fixedValue);
			}
			return isValid ? GetValidEntity() : GetInvalidEntity<CategoryLeadersEntity>().entity;
		}
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static CategoryLeadersEntity GetValidEntity(string fixedStrValue = null)
		{
			var categoryLeadersEntity = new CategoryLeadersEntity
			{
			};
			categoryLeadersEntity.PopulateAttributes();
