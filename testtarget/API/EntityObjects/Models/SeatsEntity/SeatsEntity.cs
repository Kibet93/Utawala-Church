...
namespace APITests.EntityObjects.Models
{
	public class SeatsEntity : BaseEntity
	{
		...
		/// </summary>
		/// <see cref="Utawalaaltar.Models.WorkflowStates"/>
		...
		public SeatsEntity(ConfigureOptions option)
		{
			Configure(option);
			InitialiseReferences();
		}
		...
		{
			return GetInvalidEntities<SeatsEntity>()
		...
		public override RestSharp.JsonObject ToJson()
		{
			...
			if(SeatNumber != null) 
			{
				entityVar["seatNumber"] = SeatNumber;
			}
			if(Reservation != null) 
			{
				entityVar["reservation"] = Reservation.ToString();
			}
			if (WorkflowStatesIds != default)
			{
				entityVar["workflowStatess"] = FormatManyToManyJsonList("workflowStatesId", WorkflowStatesIds);
			}
		...
		public static SeatsEntity GetEntity(bool isValid, string fixedValue = null)
		{
			if (isValid && !string.IsNullOrEmpty(fixedValue))
			{
				return GetValidEntity(fixedValue);
			}
			return isValid ? GetValidEntity() : GetInvalidEntity<SeatsEntity>().entity;
		}
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static SeatsEntity GetValidEntity(string fixedStrValue = null)
		{
			var seatsEntity = new SeatsEntity
			{
			};
			seatsEntity.PopulateAttributes();
