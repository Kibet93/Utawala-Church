...
{
	public class AdminEntity : UserBaseEntity 
	{
		...
		{
			return GetInvalidEntities<AdminEntity>()
		...
		public static AdminEntity GetEntity(bool isValid, string fixedValue = null)
		{
			if (isValid && !string.IsNullOrEmpty(fixedValue))
			{
				return GetValidEntity(fixedValue);
			}
			return isValid ? GetValidEntity() : GetInvalidEntity<AdminEntity>().entity;
		}
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static AdminEntity GetValidEntity(string fixedStrValue = null)
		{
			var adminEntity = new AdminEntity
			{
			};
			adminEntity.PopulateAttributes();
