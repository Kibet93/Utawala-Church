...
{
	public class GroupCategoryEntity : UserBaseEntity 
	{
		// 
		[EntityAttribute]
		...
		/// </summary>
		/// <see cref="Utawalaaltar.Models.CategoryGroupLeaders"/>
		public List<Guid> CategoryGroupLeadersIds { get; set; }
		public ICollection<CategoryGroupLeaderEntity> CategoryGroupLeaderss { get; set; }

		/// <summary>
		/// Outgoing one to many reference
		/// </summary>
		/// <see cref="Utawalaaltar.Models.Memberscategories"/>
		public List<Guid> MemberscategoriesIds { get; set; }
		...
		public GroupCategoryEntity()
		{
			...
			EndpointName = "group-category-entity";
			InitialiseReferences();
		}

		public GroupCategoryEntity(ConfigureOptions option)
		{
			EndpointName = "group-category-entity";
			Configure(option);
			InitialiseReferences();
		...
		{
			return GetInvalidEntities<GroupCategoryEntity>()
		...
		public override RestSharp.JsonObject ToJson()
		{
			...
			if (CategoryGroupLeadersIds != default)
			{
				entityVar["categoryGroupLeaderss"] = CategoryGroupLeaderss.Select(x => x.ToJson());
			}
			if (MemberscategoriesIds != default)
			{
				entityVar["memberscategoriess"] = Memberscategoriess.Select(x => x.ToJson());
			}
		...
		public static GroupCategoryEntity GetEntity(bool isValid, string fixedValue = null)
		{
			if (isValid && !string.IsNullOrEmpty(fixedValue))
			{
				return GetValidEntity(fixedValue);
			}
			return isValid ? GetValidEntity() : GetInvalidEntity<GroupCategoryEntity>().entity;
		}
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static GroupCategoryEntity GetValidEntity(string fixedStrValue = null)
		{
			var groupCategoryEntity = new GroupCategoryEntity
			{
			};
			groupCategoryEntity.PopulateAttributes();
