...
{
	public class AccountabilityGroupsEntity : BaseEntity
	{
		// 
		[EntityAttribute]
		...
		/// </summary>
		/// <see cref="Utawalaaltar.Models.Membersaccountabilitygroup"/>
		public List<Guid> MembersaccountabilitygroupIds { get; set; }
		public ICollection<MembersEntity> Membersaccountabilitygroups { get; set; }


		public AccountabilityGroupsEntity()
		{
			EntityName = "AccountabilityGroupsEntity";
			InitialiseReferences();
		}

		public AccountabilityGroupsEntity(ConfigureOptions option)
		{
			Configure(option);
			InitialiseReferences();
		}
		...
		{
			return GetInvalidEntities<AccountabilityGroupsEntity>()
		...
		public override RestSharp.JsonObject ToJson()
		{
			...
			if(Category != null) 
			{
				entityVar["category"] = Category;
			}
			if(LeaderID != null) 
			{
				entityVar["leaderID"] = LeaderID;
			}
			if (MembersaccountabilitygroupIds != default)
			{
				entityVar["membersaccountabilitygroups"] = Membersaccountabilitygroups.Select(x => x.ToJson());
			}
		...
		public static AccountabilityGroupsEntity GetEntity(bool isValid, string fixedValue = null)
		{
			if (isValid && !string.IsNullOrEmpty(fixedValue))
			{
				return GetValidEntity(fixedValue);
			}
			return isValid ? GetValidEntity() : GetInvalidEntity<AccountabilityGroupsEntity>().entity;
		}
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static AccountabilityGroupsEntity GetValidEntity(string fixedStrValue = null)
		{
			var accountabilityGroupsEntity = new AccountabilityGroupsEntity
			{
			};
			accountabilityGroupsEntity.PopulateAttributes();
