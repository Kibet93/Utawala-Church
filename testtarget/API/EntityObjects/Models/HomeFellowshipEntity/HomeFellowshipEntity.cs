...
{
	public class HomeFellowshipEntity : BaseEntity
	{
		// 
		[Unique]
		[EntityAttribute]
		...
		public HomeFellowshipEntity(ConfigureOptions option)
		{
			Configure(option);
			InitialiseReferences();
		}
		...
		{
			return GetInvalidEntities<HomeFellowshipEntity>()
		...
		public override Dictionary<string, string> ToDictionary()
		{
			...
			{
				{"id" , Id.ToString()},
				{"fellowshipID" , FellowshipID.ToString()},
		...
		public override RestSharp.JsonObject ToJson()
		{
			...
			{
				entityVar["fellowshipID"] = FellowshipID;
			}
			if(FellowshipName != null) 
			{
				entityVar["fellowshipName"] = FellowshipName.ToString();
			}
			if(FellowshipPastor != null) 
			{
				entityVar["fellowshipPastor"] = FellowshipPastor.ToString();
			}
			if (MembersfellowshipIds != default)
			{
				entityVar["membersfellowships"] = Membersfellowships.Select(x => x.ToJson());
			}
		...
		public static HomeFellowshipEntity GetEntity(bool isValid, string fixedValue = null)
		{
			if (isValid && !string.IsNullOrEmpty(fixedValue))
			{
				return GetValidEntity(fixedValue);
			}
			return isValid ? GetValidEntity() : GetInvalidEntity<HomeFellowshipEntity>().entity;
		}
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static HomeFellowshipEntity GetValidEntity(string fixedStrValue = null)
		{
			var homeFellowshipEntity = new HomeFellowshipEntity
			{
			};
			homeFellowshipEntity.PopulateAttributes();
