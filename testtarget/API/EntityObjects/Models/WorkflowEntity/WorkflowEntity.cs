...
namespace APITests.EntityObjects.Models
{
	public class WorkflowEntity : BaseEntity
	{
		...
		/// </summary>
		/// <see cref="Utawalaaltar.Models.Versions"/>
		public List<Guid> VersionsIds { get; set; }
		public ICollection<WorkflowVersionEntity> Versionss { get; set; }

		/// <summary>
		/// Incoming one to one reference
		/// </summary>
		/// <see cref="Utawalaaltar.Models.CurrentVersion"/>
		...
		public WorkflowEntity(ConfigureOptions option)
		{
			Configure(option);
			InitialiseReferences();
		}
		...
		{
			return GetInvalidEntities<WorkflowEntity>()
		...
		public override Dictionary<string, string> ToDictionary()
		{
			...
			{
				entityVar["currentVersionId"] = CurrentVersionId.ToString();
			}
		...
		public override RestSharp.JsonObject ToJson()
		{
			...
			if (VersionsIds != default)
			{
				entityVar["versionss"] = Versionss.Select(x => x.ToJson());
			}
			if (CurrentVersionId  != default)
			{
				entityVar["currentVersionId"] = CurrentVersionId.ToString();
			...
			foreach (var (key, guidCollection) in entityReferences)
			{
				switch (key)
				{
					case "CurrentVersionId":
						ReferenceIdDictionary.Add("CurrentVersionId", guidCollection.FirstOrDefault());
		...
		public static WorkflowEntity GetEntity(bool isValid, string fixedValue = null)
		{
			if (isValid && !string.IsNullOrEmpty(fixedValue))
			{
				return GetValidEntity(fixedValue);
			}
			return isValid ? GetValidEntity() : GetInvalidEntity<WorkflowEntity>().entity;
		}
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static WorkflowEntity GetValidEntity(string fixedStrValue = null)
		{
			var workflowEntity = new WorkflowEntity
			{
			};
			workflowEntity.PopulateAttributes();
