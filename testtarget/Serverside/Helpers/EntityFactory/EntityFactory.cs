...
using TestDataLib;
using UrlAttribute = Utawalaaltar.Validators.UrlAttribute;
...
namespace ServersideTests.Helpers.EntityFactory
{
	...
	public class EntityFactory<T> : IEntityFactory<T> where T : class, IAbstractModel, new()
	{
		...
		private bool _disableEmbeddedReferenceEntities = false;
		...
		/// <inheritdoc />
		public IEntityFactory<T> DisableEmbeddedReferenceEntities(bool disable = true)
		{
			_disableEmbeddedReferenceEntities = disable;
			return this;
		}
			...
			bool basePropertiesOnly = false)
		{
			...
			foreach (var attr in EntityFactoryReflectionCache.GetAllAttributes(entity.GetType()))
			{
				...
				if (!valueSet)
				{
					attr.SetValue(entity, GetValidAttribute(attr, fixture, context));
				}
			}
		}

		/// <summary>
		/// Get a valid attribute by checking attribute validators
		/// </summary>
		/// <param name="attr">The attribute to get value for</param>
		/// <param name="fixture">A fixture object</param>
		/// <param name="context">A SpecimenContext</param>
		/// <returns></returns>
		private object GetValidAttribute(PropertyInfo attr, ISpecimenBuilder fixture, ISpecimenContext context) 
		{
			// string
			if (attr.PropertyType == typeof(string))
			{
				// get length validator value if they exists
				var minLength = attr.GetCustomAttribute<MinLengthAttribute>()?.Length ?? 1;
				var maxLength = attr.GetCustomAttribute<MaxLengthAttribute>()?.Length ?? minLength + 10;

				// these validators can have 'length' optionally added to them
				if (attr.HasAttribute(typeof(NumericAttribute)))
				{
					return DataUtils.RandNumericString(minLength, maxLength);
				}
				if (attr.HasAttribute(typeof(AlphanumericAttribute)))
				{
					return DataUtils.RandAlphanumericString(minLength, maxLength);
				}

				// these validators can only have the 'required' validator in addition
				if (attr.HasAttribute(typeof(EmailAttribute)))
				{
					return DataUtils.RandEmail();
				}
				if (attr.HasAttribute(typeof(UuidAttribute)))
				{
					return Guid.NewGuid().ToString();
				}
				if (attr.HasAttribute(typeof(UrlAttribute)))
				{
					return DataUtils.RandUrl();
				}
			}
			// numeric
			if (attr.PropertyType == typeof(int) || attr.PropertyType == typeof(int?) ||
			         attr.PropertyType == typeof(double) || attr.PropertyType == typeof(double?))
			{
				// get length validator value if they exists
				var minRange = ((int?) attr.GetCustomAttribute<RangeAttribute>()?.Minimum) ?? 1;
				var maxRange = ((int?) attr.GetCustomAttribute<RangeAttribute>()?.Maximum) ?? minRange + 10;

				if (attr.PropertyType == typeof(int) || attr.PropertyType == typeof(int?))
				{
					return DataUtils.RandInt(minRange, maxRange);
				}
				if (attr.PropertyType == typeof(double) || attr.PropertyType == typeof(double?))
				{
					return DataUtils.RandDouble(minRange, maxRange);
				}
			}
			return fixture.Create(attr.PropertyType, context);
		}
			...
			List<(Type, string)> visited = null)
		{
			...
			foreach (var reference in references)
			{
				...
				if (!_frozenEntities.TryGetValue(reference.Type, out var referenceEntity))
				{
					...
					if (!_disableEmbeddedReferenceEntities)
					{
