...
namespace ServersideTests.Tests.Integration.BotWritten.GroupSecurityTests.Update
{
	...
	public class UsherUpdateTests : BaseUpdateTest
	{
		public UsherUpdateTests()
		{
			// % protected region % [Add constructor logic here] off begin
			// % protected region % [Add constructor logic here] end
		}

		public static TheoryData<IAbstractModel, string, string> UsherUpdateSecurityData 
		{
			get
			{
		...
		[Theory]
		[MemberData(nameof(UsherUpdateSecurityData))]
		public async Task UsherUpdateSecurityTests<T>(T model, string message, string groupName)
