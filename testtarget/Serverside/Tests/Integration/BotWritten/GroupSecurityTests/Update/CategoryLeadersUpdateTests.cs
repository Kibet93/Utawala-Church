...
namespace ServersideTests.Tests.Integration.BotWritten.GroupSecurityTests.Update
{
	...
	public class CategoryLeadersUpdateTests : BaseUpdateTest
	{
		public CategoryLeadersUpdateTests()
		{
			// % protected region % [Add constructor logic here] off begin
			// % protected region % [Add constructor logic here] end
		}

		public static TheoryData<IAbstractModel, string, string> CategoryLeadersUpdateSecurityData 
		{
			get
			{
		...
		[Theory]
		[MemberData(nameof(CategoryLeadersUpdateSecurityData))]
		public async Task CategoryLeadersUpdateSecurityTests<T>(T model, string message, string groupName)
