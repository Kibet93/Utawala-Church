...
namespace ServersideTests.Tests.Integration.BotWritten.GroupSecurityTests.Update
{
	...
	public class ProtocolUpdateTests : BaseUpdateTest
	{
		public ProtocolUpdateTests()
		{
			// % protected region % [Add constructor logic here] off begin
			// % protected region % [Add constructor logic here] end
		}

		public static TheoryData<IAbstractModel, string, string> ProtocolUpdateSecurityData 
		{
			get
			{
		...
		[Theory]
		[MemberData(nameof(ProtocolUpdateSecurityData))]
		public async Task ProtocolUpdateSecurityTests<T>(T model, string message, string groupName)
