...
namespace ServersideTests.Tests.Integration.BotWritten.GroupSecurityTests.Update
{
	...
	public class MemberUpdateTests : BaseUpdateTest
	{
		public MemberUpdateTests()
		{
			// % protected region % [Add constructor logic here] off begin
			// % protected region % [Add constructor logic here] end
		}

		public static TheoryData<IAbstractModel, string, string> MemberUpdateSecurityData 
		{
			get
			{
				var data = new TheoryData<IAbstractModel, string,string>
				{
					...
					{new MemberEntityFormTileEntity(), null, "Member"},
		...
		[Theory]
		[MemberData(nameof(MemberUpdateSecurityData))]
		public async Task MemberUpdateSecurityTests<T>(T model, string message, string groupName)
